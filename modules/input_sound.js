...
class InputSound {
  ...
  static set(description) {
    exec(
      `pactl set-default-source $(pactl list sources|grep -C2 -F "Description: ${description}"|grep Name|cut -d: -f2|xargs)`,
      { name: "KLIND OS" },
      function (error, stdout, stderr) {
        if (error) {
          console.log(stderr);
  ...
  static getDefault(callback) {
    exec(
      `pactl list sources | grep -A1 "Name: $(pactl info | grep "Default Source" | awk '{print $3}')" | grep "Description" | awk -F": " "{print \$2}"`,
      { name: "KLIND OS" },
      function (error, stdout, stderr) {
        if (error) {
          throw new Error("Filed to set device!");
