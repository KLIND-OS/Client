...
class ___lowlevelapi_volume_devices {
  ...
  static getDefault(callback) {
    exec(
      `pactl list sinks | grep -A1 "Name: $(pactl info | grep "Default Sink" | awk '{print $3}')" | grep "Description" | awk -F": " "{print \$2}"`,
      { name: "KLIND OS" },
      function (error, stdout, stderr) {
        if (error) {
  ...
  static set(description) {
    exec(
      `pactl set-default-sink $(pactl list sinks|grep -C2 -F "Description: ${description}"|grep Name|cut -d: -f2|xargs)`,
      { name: "KLIND OS" },
      function (error, stdout, stderr) {
        if (error) {
          throw new Error("Filed to set device!");
