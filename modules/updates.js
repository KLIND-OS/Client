        ...
        LowLevelApi.Branch.getSelected(branch => {
          ...
          exec(`pacman -Suy --noconfirm`, (error) => {
              ...
              exec(`rm -rf /root/klindos-server/data`, (error) => {
                  ...
                  exec(`git clone --branch ${branch} --depth 1 https://github.com/JZITNIK-github/KLIND-OS-Server /root/klindos-server/data`, (error) => {
                      if (error) {callback("0%", "Error!"); return }
                      callback("50%", "Aktualizace klienta")
                      exec(`git clone --branch ${branch} --depth 1 https://github.com/JZITNIK-github/KLIND-OS-Client /root/KLIND-OS-Client`, (error) => {
                          if (error) {callback("0%", "Error!"); return }
